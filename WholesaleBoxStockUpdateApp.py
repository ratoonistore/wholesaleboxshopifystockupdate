...
mailSubject = "WholesaleBox Job Run Started : " + datetime.now().strftime('%d-%m-%Y %H:%M:%S')
mailBody = "WholesaleBox Job Run Started : " + datetime.now().strftime('%d-%m-%Y %H:%M:%S')
...
for i in range(1, 2):
    products = shopify.Product.find(vendor = "Wholesalebox", limit = productLimit, page = 23)
...
mailSubject = "WholesaleBox Job Run Report for : " + datetime.now().strftime('%d-%m-%Y %H:%M:%S')
mailBody = """<html>
      <head></head>
      <body>
        <h3>""" + mailSubject + """</h3>
